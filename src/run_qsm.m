...
odir = "../../../2019_FrenchGuiana/TLS_segmentation/tls2trees/qsm/clouds";

dirs = dir(data_dir);

for i=1:size(dirs, 1)
    % if dirs(i).isdir && (dirs(i).name ~= "." && dirs(i).name ~= ".." && dirs(i).name ~= "._.DS_Store" && dirs(i).name ~= "0.1" && dirs(i).name ~= "0.2")
    if str2double(dirs(i).name) >= 0.0 && str2double(dirs(i).name) <= 2.2
    % if str2double(dirs(i).name) == 0.9
        ...
        if size(files, 1) > 0
            ...
            for j=1:size(files, 1)
                ...
                % Ps.("ti_" + tree_name) = P;
                shifts.("ti_" + tree_name) = mean(P);
            end
            
            % disp("Saving clouds .mat...");
            dir_label = strrep(dirs(i).name, ".", "-");
            qsm_name = strcat("paracou_", dir_label);
            qsm_file = fullfile(odir, qsm_name);
            % save(qsm_file, "-struct", "Ps");
            
            disp("Saving global shifts...");
            shifts_file = fullfile(odir, strcat(qsm_name, "_shifts"));
